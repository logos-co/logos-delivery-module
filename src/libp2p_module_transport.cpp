...
public:
    ...
    explicit Libp2pModuleTransport(const LogosModuleContext& context)
    ...
    StdLogosResult call(const std::string& method, const nlohmann::json& args) override
    {
        ...
        auto peer = m_modules->bind_libp2p(kLibp2pModuleName);
...
{
    return std::make_shared<Libp2pModuleTransport>(context);
}
