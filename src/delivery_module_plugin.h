...
public:
     ...
     *
     * Optional `"libp2pProvider": "libp2p_module"` runs the protocols over that module's libp2p node, see `docs/libp2p-provider.md`.
     *
...
private:
    ...
    StdLogosResult startNetBridge(const std::string& provider);
    void stopNetBridge();
    StdLogosResult abortCreateNode(const std::string& reason);
    StdLogosResult verifySharedIdentity(const std::string& provider,
