...
The optional top-level `"libp2pProvider": "libp2p_module"` runs the delivery
protocols over the libp2p node `logos-libp2p-module` already owns, instead of
starting one. Both modules need the same key, and it covers the Edge protocols:
see [docs/libp2p-provider.md](docs/libp2p-provider.md).
