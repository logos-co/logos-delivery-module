// Bridged mode (useLibp2pModule): mocks/net_bridge_mock.cpp fakes the provider and the ABI, so the op allow-list in src/libp2p_module_transport.cpp is out of reach here.
...
std::string bridgedCfg()
{
    return json{{"mode", "Edge"}, {"preset", "logos.test"}, {"useLibp2pModule", true}}.dump();
}
...
LOGOS_TEST(bridged_createNode_registers_the_net_backend) {
    ...
    LOGOS_ASSERT_TRUE(net::registeredName() == kLibp2pModuleName);
